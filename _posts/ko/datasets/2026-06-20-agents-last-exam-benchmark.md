...
**agents-last-exam**은 컴퓨터 사용(computer-use) 에이전트를 장기 과제(long-horizon task) 기준으로 평가하기 위한 벤치마크 데이터셋입니다. 총 153개 과제로 구성되며 Business, Computing, Mathematics, Engineering, Legal 등 여러 전문 도메인을 가로질러 에이전트가 실제 업무 워크플로를 수행할 수 있는지 측정합니다.
...
1. 도메인별 과제 필터링: `category`와 `subdomain`으로 평가 대상 범위를 정합니다.
2. 도구 환경 구성: `software` 필드에 명시된 도구를 과제별로 컨테이너 환경에 준비합니다.
3. 에이전트 실행: 각 과제의 `task_prompt`를 입력으로 에이전트를 실행합니다.
4. 출력물 수집: 에이전트가 생성한 파일이나 응답을 과제별 출력 디렉터리에 저장합니다.
5. 검증 실행: `agent_must_do` 필드의 행동 목록을 기준으로 완수 여부를 자동 채점합니다.
