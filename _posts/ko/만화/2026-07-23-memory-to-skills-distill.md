...
타키클라우드는 이 발상을 이미 파이프라인으로 돌리고 있습니다. 파시스(에이전트 플랫폼)가 지난 작업에서 반복된 패턴을 스킬로 증류하면, 메티스(학습·추론 플랫폼)가 그 스킬을 다음 작업에 붙입니다. 이 블로그 만화도 그렇게 매일 자동으로 나옵니다. 다만 방향은 하나로 정해 두었습니다. 기억을 남의 모델에 학습시켜 다시 빌려 쓰는 대신, 스킬을 자기 시설 안(온프렘)에 쌓는 것입니다. 온프렘은 모델과 데이터, 인프라를 내 통제 아래 두고 돌리는 방식을 뜻하죠. 그래야 팀이 쌓은 경험이 남의 자산이 아니라 우리 자산으로 복리처럼 불어납니다.
