...
다키클라우드는 아이기스(Aegis)로 모델과 데이터를 고객 시설 안에 두고, 그 안에서 파시스(Paxis)가 에이전트 업무를 돌립니다. 밖으로 나가는 출시본이 없으니 남의 검사 일정에 우리 일정이 묶이지 않습니다. 대신 검사 주체가 우리가 되는 만큼, 누가 어떤 모델을 언제 호출했는지 시그넘(Signum)의 권한과 감사 기록으로 남깁니다.
