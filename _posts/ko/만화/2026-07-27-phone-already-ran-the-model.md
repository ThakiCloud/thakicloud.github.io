...
온디바이스는 결국 크기를 줄인 온프렘입니다. 온프렘은 모델과 데이터를 남의 시설이 아니라 내 시설 안에서 굴리는 방식이고, 폰이든 사내 랙이든 발상은 똑같아요. 타키클라우드는 그 발상을 회사 규모로 옮겨 놓았습니다. 메티스가 학습과 서빙을 고객 시설 안 GPU에서 끝내고, 파시스가 그 위에서 에이전트를 굴립니다.
