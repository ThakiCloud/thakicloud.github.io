...
toc_icon: "cog"
toc_sticky: true
canonical_url: "https://thakicloud.com/tech-blog/ko/news/ai-ebook-launch-skill-pipeline/"
reading_time: true
categories:
  - news
...
입력은 틈새 주제 하나입니다. 출력은 책 한 권이 아니라 **판매 키트 한 벌**입니다. 표지가 박힌 이북 PDF, 30일치 소셜 콘텐츠 캘린더, 세일즈 페이지, 리드 마그넷, 이메일 시퀀스, 그리고 배포 스크립트까지 한 폴더에 떨어집니다. 책을 쓰는 도구가 아니라, 책을 팔 준비까지 하는 도구라고 보는 편이 정확합니다.

폴더 구조를 보면 스킬의 사고방식이 드러납니다.
...
**2. 콘텐츠 엔진.** 30일치 유기적 소셜 캘린더를 만듭니다. 포스트마다 후킹 유형과 CTA가 붙고, X API 포맷에 맞는지 검증을 거칩니다. 사람이 쓴 듯한 톤을 크리에이터 스타일에 맞춰 흉내 냅니다.

**3. 제품, 즉 책 본체.** 여기가 무겁습니다. 8~12개 챕터 아웃라인을 잡고, 챕터별로 서브에이전트를 띄워 본문을 씁니다. 챕터마다 새 에이전트를 쓰는 이유는 컨텍스트 위생입니다. 한 세션에 모든 챕터를 욱여넣으면 뒤로 갈수록 품질이 흐려집니다. 표지 이미지는 gpt-image-2로 만드는데, 프롬프트에서 글자를 빼는 게 핵심입니다. 한글이 이미지 안에서 깨지기 때문에, 배경만 생성하고 제목은 파이썬 스크립트가 폰트로 얹습니다. 한국어판은 Apple SD Gothic Neo를 씁니다. 마지막에 `generate_ebook.py`가 표지와 본문을 합쳐 `ebook.pdf`를 뽑습니다.

**4. 세일즈 자산.** 헤드라인, 문제와 해법, 가격, FAQ, 보증 문구가 담긴 세일즈 카피를 만들고, 그 JSON에서 HTML 세일즈 페이지를 자동 생성합니다. 3~4챕터짜리 리드 마그넷과 5통짜리 이메일 너처 시퀀스도 같이 나옵니다.
...
실제 산출물 PDF 세 권을 Google Drive에 공개로 올려 두었습니다. 위 "PDF 보기" 링크로 스킬이 뽑은 결과물을 직접 확인할 수 있습니다.
...
출력은 PDF뿐입니다. EPUB도, KDP 네이티브 포맷도 없습니다. 다섯 권 모두 한국어입니다. 다른 언어 파이프라인은 있지만 이번 실측에는 없었습니다. 배포는 스크립트가 준비됐을 뿐, 실제 스토어에 올라간 라이브 배포나 아마존 KDP 등록은 확인되지 않았습니다. 결제와 전달은 Whop API와 직접 PDF 전달을 가정한 직판 모델이고, 전통적 서점 유통은 스킬 범위 밖입니다. 가격 수수료 같은 숫자는 일부 미검증이라 도입 전 원문 확인이 필요합니다.
