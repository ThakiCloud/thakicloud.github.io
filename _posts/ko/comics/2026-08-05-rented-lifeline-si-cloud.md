...
재판매 매출은 잘 굴러갈 때도 마진이 남의 요금표에 묶여 있습니다. ThakiCloud가 파시스(Paxis)를 중심에 놓는 이유가 여기 있죠. 고객 업무를 에이전트로 자동화해 성과 자체를 만들어내면 값을 매기는 기준이 남의 인스턴스 시간이 아니라 우리가 끝낸 일이 됩니다. 그 에이전트가 태우는 토큰은 메티스(Metis)가 서빙에서 붙잡습니다. 데이터와 모델이 밖으로 나가면 곤란한 고객은 아이기스(Aegis)로 자기 시설 안에 그대로 앉힙니다. 밥줄은 빌리는 것보다 심는 편이 오래갑니다.
