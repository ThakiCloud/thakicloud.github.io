---
title: "챗봇은 말만 했지, 에이전트는 눌러"
excerpt: "정부 AI가 답만 하던 시절을 졸업했습니다. 이제 버튼을 누르는데, 그 버튼이 어느 건물에 있는지는 아무도 안 물어봤습니다."
date: 2026-08-12
categories:
  - comics
tags:
  - 정부AI
  - AI에이전트
  - 공공클라우드
  - 온프렘
  - 감사추적
author_profile: true
toc: false
image: /assets/images/posts/만화/government-ai-goes-agent/strip.webp
video: /assets/videos/posts/만화/government-ai-goes-agent/comic.mp4
audiobook: /assets/audio/posts/government-ai-goes-agent/audiobook-ko.mp3
audiobook_note: "만화 캐릭터 목소리로 듣는 오디오북 (Qwen3-TTS 로컬)"
---

범정부 AI 공통기반은 부처마다 따로 만들던 AI를 한 번 지어 같이 쓰자는 정부 공용 토대입니다. 이번 소식의 핵심은 그 토대가 챗봇 단계를 지나 에이전트 단계로 넘어간다는 사실이고요. 챗봇은 물어보면 답만 했습니다. 반면 에이전트는 서류를 열고 조회하고 다음 절차를 실제로 밟습니다. 여기에 최신 민간 모델까지 얹기로 한 결정이 흥미롭습니다. 성능은 확실히 올라가는데 그 성능의 소유권과 갱신 주기는 밖에 있습니다.

![챗봇은 말만 했지, 에이전트는 눌러](/assets/images/posts/만화/government-ai-goes-agent/strip.webp)

> 원 뉴스: [범정부 AI 공통기반, 챗봇 넘어 에이전트로…최신 민간 모델 품는다](https://n.news.naver.com/mnews/article/092/0002433460) · bespin

**▶ 만화 영상판, 캐릭터들이 직접 말합니다**

<video controls playsinline preload="metadata" poster="/assets/images/posts/만화/government-ai-goes-agent/strip.webp" style="width:100%;border-radius:8px;">
  <source src="/assets/videos/posts/만화/government-ai-goes-agent/comic.mp4" type="video/mp4">
  <track kind="subtitles" srclang="ko" label="한국어" src="/assets/videos/posts/만화/government-ai-goes-agent/comic.ko.vtt" default>
</video>

[영상 다운로드](/assets/videos/posts/만화/government-ai-goes-agent/comic.mp4)

## ThakiCloud 제품 적용 시사점

에이전트가 실제로 절차를 밟기 시작하면 질문이 바뀝니다. 이 모델이 똑똑하냐가 아니라, 누가 무슨 권한으로 무엇을 눌렀는지가 남느냐로요. 파시스(Paxis)는 업무 자동화를 사람 승인 단계와 감사 기록 위에 올려놓고 굴립니다. 눌린 버튼과 바뀐 값은 시그넘(Signum)이 신원과 권한으로 붙잡아 둡니다. 모델 자체는 최신으로 바꿔 끼워도 괜찮습니다. 다만 그 모델이 돌아가는 자리와 결재 기록만큼은 아이기스(Aegis)처럼 기관 시설 안에 두는 편이 마음 편합니다. 두뇌는 필요할 때 갈아 끼우고 실행과 책임은 우리 안에 두는 구조입니다. 공공에는 그게 현실적인 답이라고 봅니다.

---

*이 만화는 업계 뉴스를 바탕으로 자동 생성된 초안입니다.*
