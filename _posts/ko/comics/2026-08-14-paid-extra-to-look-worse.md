...
재미있는 건 가격입니다. 30초에 4달러 12센트가 붙었습니다. 1분이면 8달러가 넘고 마음에 안 들어 다시 뽑을 때마다 같은 금액이 또 나갑니다. 해상도를 낮춘다고 계산량이 알아서 줄지도 않습니다. 그러니 거친 질감은 절약이 아니라 취향에 가깝습니다. 프롬프트는 공짜로 공유되는데 렌더링 청구서는 각자 부담이라는 게 이 장르의 진짜 규칙이더군요.
...
영상 생성은 결국 GPU 추론입니다. 프레임 하나가 곧 연산입니다. 이 연산을 누구 카드에서 돌리느냐가 단가를 정합니다. 남의 API에 얹으면 프레임이 그대로 청구서가 되지만, Metis에서 우리 카드로 서빙하면 같은 프레임이 전기와 감가상각으로 바뀝니다. 컷을 스무 번 다시 뽑아도 겁나지 않는 이유가 여기 있어요.
카드를 실제로 대는 층이 Telox와 Velox입니다. 실험은 남는 GPU로 돌리고 정기 렌더는 전용 자원에 붙이는 식으로 나눕니다. 그러면 창작 실험의 비용 곡선이 사용량이 아니라 보유량을 따라갑니다. 프롬프트 설계와 렌더, 실패 재시도, 자막 붙이기까지 묶어 자동으로 굴리는 건 Paxis 몫이고요. 이 만화와 영상도 파이프라인에서 나옵니다. 감성은 취향껏 거칠게 가되, 미터기까지 남의 것일 필요는 없습니다.
