...
프롬프트 엔지니어링에서 시작한 이름표가 컨텍스트, 하네스, 루프를 지나 이제 그래프 엔지니어링까지 왔습니다. 새 용어가 나올 때마다 업계는 혁명이라고 부르지만 뜯어보면 하는 일은 하나로 모입니다. 프롬프트는 모델에게 말을 어떻게 거느냐를 정합니다. 컨텍스트는 매 순간 무엇을 보여줄지 고르는 일입니다. 하네스는 도구와 검증 규칙을 감싸는 골격입니다. 루프는 결과를 다시 입력으로 넣어 수렴시키는 반복입니다. 그래프는 반복들을 노드와 화살표로 이어 설계도로 만듭니다. 즉 다섯 개 학문이 아니라 같은 문제를 다섯 번 다시 부른 셈입니다. 그런데 강의는 다섯 번 팔렸습니다.
...
이름이 뭐로 바뀌든 마지막 질문은 똑같이 남습니다. 이 루프는 누구 기계에서 도나요. 다키클라우드는 이 질문에 먼저 답하는 쪽을 골랐습니다. 파시스는 에이전트를 실제로 풀고 결과를 다시 물려 수렴시키는 오케스트레이션을 맡고 메티스는 이 반복이 도는 GPU와 데이터를 회사 시설 안에 둡니다. 온프렘은 남의 데이터센터가 아니라 자기 랙에서 돌린다는 뜻입니다. 그래야 학습 데이터와 로그가 밖으로 나가지 않습니다. 이 블로그도 같은 구조로 굴러갑니다. 뉴스를 고르고 대본을 쓰고 그림과 영상을 뽑는 과정이 전부 사내 에이전트 루프고 이 루프를 부르는 이름은 올해만 다섯 번 바뀌었지만 랙은 그대로 있습니다. 새 용어를 따라잡느라 지치셨다면 용어 대신 인프라의 주인이 누구인지부터 확인해 보세요.
