...
![같은 면적의 주차장이라도 경차가 오느냐 대형버스가 오느냐에 따라 받을 수 있는 대수가 달라진다는 그림](/assets/images/inference-tuning-is-measurement-slide-01.webp)

## 무엇을 해봤나

엔비디아 B200 한 장에 270억 개짜리 모델을 4비트로 압축해 올렸습니다. 압축을 양자화(quantization)라고 부르는데, 무게를 줄여 카드 한 장에 들어가게 만드는 작업입니다.

이 모델은 층마다 성격이 다릅니다. 64개 층 가운데 16개 층만 대화가 길어질수록 [기억 공간, 즉 KV 캐시](https://docs.vllm.ai/en/stable/features/automatic_prefix_caching.html)를 더 씁니다. 나머지 48개 층은 길이와 무관하게 일정한 공간만 씁니다. 덕분에 100만 토큰짜리 문맥이 카드 한 장에 들어갑니다. 모든 층이 기억 공간을 쓰는 보통 구조였다면 137기가바이트가 필요해서 한 장으로는 불가능합니다.

여기에 [초안 작성기(drafter)](https://docs.vllm.ai/en/stable/features/speculative_decoding.html)를 붙였습니다. 작은 모델이 다음 토큰을 미리 여러 개 제안하고 큰 모델이 한 번에 검증하는 방식입니다. 저희 실측에서 제안한 토큰 열 개 중 세 개가 채택됐고 한 번 계산할 때마다 평균 3.4개 토큰이 나왔습니다.

그리고 두 엔드포인트에서 [동시 접속, 즉 max_num_seqs](https://docs.vllm.ai/en/stable/configuration/engine_args.html)를 1부터 256까지 올려가며 쟀습니다. 설정은 한 번에 하나씩만 바꿨습니다. 그래야 무엇이 무엇을 바꿨는지 말할 수 있습니다.
...
![전체 64개 층 가운데 16개 층이 기억 공간의 99.5퍼센트를 쓰고 나머지 48개 층이 0.5퍼센트만 쓴다는 도식](/assets/images/inference-tuning-is-measurement-slide-02.webp)
...
**남는 메모리는 안전 마진입니다. 저희는 그것을 한 번 어겼습니다.** 엔진은 [gpu_memory_utilization](https://docs.vllm.ai/en/stable/configuration/engine_args.html)을 근거로 "남은 공간을 다 쓰라"며 정확한 숫자까지 알려줍니다. 저장 형식을 바꾼 뒤 그 숫자를 그대로 넣었더니 서버가 못 뜨고 재시작만 반복했습니다. 1기가바이트가 모자랐습니다. 엔진이 준 그 숫자는 형식을 바꾸기 전에 잰 값이었습니다. 형식이 바뀌면 다른 부분이 조금 더 필요해집니다. 결국 6기가바이트를 남기는 값으로 정착했습니다.

즉, 사람 말로는 설정을 최대로 밀어붙이는 것과 최적으로 맞추는 것은 다른 일이라는 뜻입니다.

![엔진이 권하는 최대치까지 쓰면 재시작이 반복되고 6기가바이트를 남겨야 안정된다는 비교 그림](/assets/images/inference-tuning-is-measurement-slide-03.webp)
...
![동시 접속을 8에서 256까지 올리면 전체 처리량은 2,900 토큰 근처에서 멈추고 요청당 속도만 무너지는 그래프](/assets/images/inference-tuning-is-measurement-slide-04.webp)
...
![실제 트래픽 693건 가운데 70퍼센트가 20만 토큰 이하이고 50만 토큰을 넘는 요청은 없었다는 원그래프](/assets/images/inference-tuning-is-measurement-slide-05.webp)
...
![동시 입차 대수는 주차장 면적을 평균 차 크기로 나눈 값이라는 결론 슬라이드](/assets/images/inference-tuning-is-measurement-slide-06.webp)
