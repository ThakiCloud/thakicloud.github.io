...
seo_description: "B200 한 장에 27B 모델을 4비트로 올려 동시 접속 1에서 256까지 실측했습니다. 양자화·투기적 디코딩·메모리 설정을 워크로드에 맞추면 요청당 100 토큰/초를 지키면서 동시 32명을 받습니다."
date: 2026-09-05
published: true
categories:
  - llmops
...
**동시 입차 대수를 늘려도 주차장이 줄지 않았습니다.** 설정값을 8에서 32로 네 배 올렸는데 기억 공간이 0.03퍼센트밖에 안 움직였습니다. 이 모델 구조에서는 공간을 미리 통째로 잡아두기 때문입니다.
...
**남는 메모리는 안전 마진이었습니다.** 엔진이 "남은 공간을 다 쓰라"고 권하지만 저희는 17기가바이트를 남겼습니다. 그 여유가 없으면 갑자기 큰 요청이 들어왔을 때 서버가 죽습니다. 느려지는 정도로 끝나지 않습니다.

즉, 사람 말로는 설정을 최대로 밀어붙이는 것과 최적으로 맞추는 것은 다른 일이라는 뜻입니다.

## 나온 숫자

설정을 맞춘 뒤 카드 한 장에서 나온 결과입니다. 초당 출력 토큰 기준이고 각 단계마다 다섯 번 재서 가운데 값을 썼습니다.
