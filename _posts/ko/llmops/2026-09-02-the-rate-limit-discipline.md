...
excerpt: "트래픽이 폭주하면 서비스가 죽는다고 생각하기 쉽지만, 실제 원인은 대개 시스템 스스로 부른 두 번째 부하, 즉 재시도 폭풍입니다. 이 글은 과부하가 어떤 순환으로 치닫는지를 짚고, 서버의 레이트 리미트와 클라이언트의 백오프, 그리고 LLM API의 예산 통제로 그 순환을 세 지점에서 끊는 방법을 숫자를 대며 확인합니다. 일부러 늦추고 거절하는 규율이 과부하 대응의 전부인 이유를 다룹니다."
seo_title: "재시도 폭풍이 서비스를 무너뜨리는 이유와 레이트 리미팅, 백오프, LLM 예산 통제로 대응하는 법"
seo_description: "과부하에서 서비스를 무너뜨리는 건 외부 트래픽이 아니라 내 시스템의 재시도 폭풍입니다. 토큰 버킷과 429, 지수 백오프와 지터, LLM API 예산 통제로 캐스케이드 실패의 순환을 끊는 실무 방법을 정리합니다."
date: 2026-09-02
...
ebook_pages: 34
audiobook: "https://drive.google.com/file/d/1AR0CFjvXKNB2kQhm9EawA5hIT1gl4J9f/view"
...
<!-- nlm-visual -->
![핵심 개념 요약 인포그래픽 1](/assets/images/posts/news/the-rate-limit-discipline/nlm-infographic-1.webp)
