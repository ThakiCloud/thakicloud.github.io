...
ThakiCloud의 AI 플랫폼은 K8s 위에서 Kueue로 GPU 워크로드를 스케줄링하고, vLLM으로 멀티테넌트 추론을 서빙하며, 멀티테넌트 에이전트를 운영합니다. Ornith-1.0 같은 모델이 이 스택에 의미가 있는 지점은 크게 세 가지입니다.

첫째, **자체 코딩 에이전트의 모델 주권**입니다. 코드는 가장 민감한 자산 중 하나입니다. 폐쇄 API에 사내 코드를 흘려보내지 않고, 온프레미스 또는 전용 테넌트에서 코딩 에이전트를 돌리려는 수요는 금융·공공·방산처럼 데이터 반출이 제약되는 환경에서 특히 큽니다. MIT 라이선스 + 자가호스팅 가능한 가중치는 이 요구에 정확히 맞습니다. 262K 컨텍스트와 OpenAI 스타일 tool_calls 출력은 기존 에이전트 루프에 베이스 모델만 교체해 꽂을 수 있게 해 줍니다.

둘째, **크기 스펙트럼이 멀티테넌트 비용 셈법과 맞물립니다.** 9B는 가벼운 보조 작업이나 테넌트별 저비용 티어에, 35B MoE는 활성 파라미터 대비 효율이 좋은 주력 서빙에, 397B는 고난도 작업 전용 풀에 배치하는 식의 계층화가 가능합니다. Kueue로 GPU를 분할 스케줄링하는 환경에서는 "어떤 테넌트에 어떤 크기를 얼마의 우선순위로" 붙일지가 곧 단가입니다. 한 패밀리 안에서 크기를 고를 수 있다는 것은 운영 유연성을 그대로 비용 절감으로 환산할 수 있다는 뜻입니다.

셋째, **자가-스캐폴딩이라는 방법론 자체가 우리에게 시사점입니다.** ThakiCloud는 다양한 고객 환경에서 도메인 특화 에이전트를 운용합니다. 사람이 작업별 스캐폴드를 손으로 깎는 비용은 도메인이 늘수록 선형으로 증가합니다. "발판을 학습 대상으로 끌어올린다"는 발상은, 도메인 에이전트의 프롬프트·도구 규약을 사람이 매번 튜닝하는 대신 보상 신호로 자동 개선하는 내부 파이프라인을 설계할 때 직접 빌려올 수 있는 패턴입니다. 모델을 그대로 쓰지 않더라도, 메커니즘은 우리 워크플로 엔진에 이식 가능한 아이디어입니다.
