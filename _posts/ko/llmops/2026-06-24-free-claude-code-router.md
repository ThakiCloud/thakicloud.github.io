...
```mermaid
flowchart TB
    CC["Claude Code<br/>(클라이언트)"]
    CX["Codex<br/>(클라이언트)"]
    FP["FastAPI 프록시<br/>(로컬 서버)"]
    EP1["/v1/messages<br/>Anthropic 호환"]
    EP2["/v1/responses<br/>OpenAI 호환 → 변환"]
    MR{"모델 라우터<br/>MODEL_OPUS / SONNET / HAIKU"}
    NZ["정규화기<br/>(thinking·tool_use·에러 변환)"]
    CLOUD["클라우드 프로바이더<br/>NVIDIA NIM · OpenRouter · Gemini<br/>DeepSeek · Mistral · Groq 등 14개"]
    SELF["자체호스팅 백엔드<br/>Ollama · LM Studio<br/>llama.cpp / vLLM"]
