...
date: 2026-07-11
lang: ko
canonical_url: "https://thakicloud.github.io/ko/llmops/gguf-quantization-internals/"
categories:
  - llmops
---

![양자화된 신경망 가중치가 크기가 다른 블록으로 재배열되는 모습을 표현한 추상 일러스트](/assets/images/gguf-quantization-internals-hero.png)
...
![Qwen2.5-0.5B의 Q4_K_M 파일을 해부한 결과 진짜 4비트 텐서는 6.1퍼센트뿐이고 실효 비트폭은 6.16비트였다는 것을 보여주는 슬라이드](/assets/images/gguf-quantization-internals-slide-02.png)
...
![레거시 계열(Q4_0, Q5_0)은 32개 가중치를 한 블록으로 묶고 K-양자화 계열(Q4_K, Q6_K)은 256개를 슈퍼블록으로 묶어 스케일과 최솟값을 세분화한다는 비교 슬라이드](/assets/images/gguf-quantization-internals-slide-05.png)
...
![Qwen2.5-0.5B의 양자화 레벨별 파일 크기와, Q4_K_M 내부의 실제 텐서 타입 구성을 보여주는 차트. Q4_K_M의 실효 비트폭은 6.16으로 이름표의 4.0과 크게 다릅니다](/assets/images/gguf-quantization-internals-results.png)
...
![ffn_down은 열 개수 4864가 256의 배수라 Q6_K에 성공하지만 token_embd와 attn_q는 열 개수 896이 256의 배수가 아니라 Q5_0으로 폴백한다는 것을 보여주는 슬라이드](/assets/images/gguf-quantization-internals-slide-07.png)
...
![GGUF 모델을 Paxis 샌드박스에서 텐서 단위로 집계하고 실효 비트폭을 실측한 뒤 정책 게이트와 감사 로그를 거쳐 ai-platform의 GPU 메모리 스케줄링으로 이어지는 파이프라인 슬라이드](/assets/images/gguf-quantization-internals-slide-10.png)
...
![히든 사이즈 896인 소형 모델은 실효 비트폭이 6.16으로 튀지만 히든 사이즈 4096인 대형 모델은 K-양자화가 정상 적용돼 약 4.8비트로 이름표에 가깝다는 것을 대비한 슬라이드](/assets/images/gguf-quantization-internals-slide-08.png)
