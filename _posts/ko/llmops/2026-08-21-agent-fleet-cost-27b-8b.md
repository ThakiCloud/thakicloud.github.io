---
title: "에이전트 플릿 하루치를 정가로 환산했더니 2,202달러였고, 그중 90%는 캐시 읽기였습니다"
excerpt: "무인 에이전트 57개가 하루에 쓰는 토큰을 상용 API 정가로 환산하면 2,202달러입니다. 비용 구조를 뜯어보니 출력 토큰은 172달러뿐이었고 나머지는 캐시 읽기였습니다. 자가 서빙에서는 그 축이 아예 과금되지 않습니다."
categories:
  - llmops
tags:
  - agent-platform
...
사내 에이전트를 상용 API로 돌리고 있고 청구서가 계속 커지는 중이라면, 어디를 먼저 손대야
하는지 알려드리려고 씁니다. 결론부터 적겠습니다. **비용의 90%는 출력 토큰이 아니라 캐시
읽기였고, 자가 서빙에서는 그 축이 과금 대상이 아닙니다.** 그리고 제일 큰 레버는 모델 교체가
아니라 서빙 설정 두 줄이었습니다.
...
132달러입니다. 정가 환산 2,202달러와 비교하면 **16.7배**이고, 월로 보면 66,064달러가
3,960달러가 됩니다.
...
월 정가 환산액이 66,064달러라면 어느 카드든 회수기간이 한 달 안쪽으로 나옵니다. 다만 이
계산은 **매일 최대 사용일 수준으로 돌린다는 가정**이고, 저희 실결제는 구독이라 정가를 그대로
