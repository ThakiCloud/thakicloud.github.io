...
여기서 이 글 초판이 크게 틀렸습니다. 저는 오프로드 디코딩을 1.2~3.8 tok/s로 보고하며 "배치 전용, 너무 느림"이라 단정했는데, 그건 ktransformers가 아니라 최적화 이전 경로를 잰 값이었습니다. 제가 쓴 llama.cpp `--n-cpu-moe`는 전문가를 CPU에 두는 배치만 같을 뿐 AMX INT4 커널도 MLA도 CUDA graph도 타지 않고, 또 다른 측정은 랜덤 BF16 가중치로 전문가 연산만 외삽한 값이었습니다. 실제로 SOSP25 논문에는 최적화 이전 baseline이 decode 4.68 tok/s, GPU 활용 30% 미만으로 적혀 있는데, 제 1.2~3.8은 정확히 그 바닥이었습니다. 원저자가 공개한 값은 다릅니다. DeepSeek-V3/R1을 q4km(INT4)으로 RTX 4090 24GB와 듀얼 Xeon Gold 6454S(382GB~1TB DRAM)에서 돌리면 디코딩이 최대 약 14~16 tok/s입니다.

제가 빠뜨린 4배가 무엇인지 정식 스택으로 다시 확인했습니다. Xeon Platinum 8480+(Sapphire Rapids, AMX 지원)와 2TB RAM, H100 위에 ktransformers의 kt-kernel(0.6.3, 소스 빌드)을 올려, DeepSeek-V3 실제 형상(히든 7168, MoE 중간 2048, 활성 전문가 8개, MoE 58개 층)의 디코딩을 커널만 바꿔 가며 쟀습니다.
...
INT4 커널은 BF16보다 3.9배, AVX2보다 4.2배 빨랐고, 초판이 보고한 1.2~3.8은 위 표의 BF16과 AVX2 줄과 정확히 겹칩니다. 이유는 단순합니다. 디코딩은 토큰마다 활성 전문가의 가중치를 RAM에서 읽어 오는 대역폭 병목 구간인데, INT4는 BF16보다 바이트를 4분의 1만 읽으면 됩니다. 제가 놓친 것이 바로 이 4배였습니다. 이 12.4 tok/s는 MoE 층만 CPU에서 스레드 60개로 튜닝한 값이고(112개로 늘리면 NUMA 동기화 비용에 오히려 6.6으로 떨어졌습니다), 실제 서빙에서는 어텐션과 shared expert가 GPU에서 겹쳐 돌아 원저자의 14~16 tok/s와 같은 대역에 놓입니다. 한 자릿수 초반이 아니라 두 자릿수 초반, 준-인터랙티브가 맞습니다.
...
| Offload (공식) | 4090 24GB + 듀얼 Xeon | 약 $3 | 약 14~16 tok/s |
| Offload (초판 프록시) | AMX 서버 + GPU 1장 | 약 $3 | 1.2~3.8 tok/s |
