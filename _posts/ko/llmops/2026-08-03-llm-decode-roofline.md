...
![좁은 발광 통로 하나가 거대한 연산 큐브 격자와 작은 클러스터를 잇고 있는 추상 이미지](/assets/images/llm-decode-roofline-hero.webp)
...
![프리필 대비 디코드 한 스텝이 16GB 가중치를 읽어 토큰 한 개를 만드는 구조를 막대 길이로 대비한 슬라이드](/assets/images/llm-decode-roofline-slide-03.webp)
...
![메모리 대역폭 사선과 연산 한계 수평면이 만나는 능선을 입체로 표현하고 가로축이 배치 크기임을 표시한 슬라이드](/assets/images/llm-decode-roofline-slide-04.webp)
...
![PagedAttention, 청크 프리필, RadixAttention이 각각 배치 공간 확보와 스케줄링과 연산 생략으로 이어지는 흐름을 나열한 슬라이드](/assets/images/llm-decode-roofline-slide-08.webp)
...
![배치 1의 0.834ms와 배치 16의 0.920ms를 나란히 놓고 시간 증가량이 10%에 불과함을 강조한 슬라이드](/assets/images/llm-decode-roofline-slide-06.webp)
...
![왼쪽은 산술 강도 대비 달성 연산 성능을 로그 좌표에 그린 루프라인 그래프, 오른쪽은 배치 크기에 따라 토큰당 시간이 834µs에서 19.6µs로 떨어지는 곡선](/assets/images/llm-decode-roofline-results.webp)
...
![전용 GPU 할당과 큐 기반 다중 테넌트를 좌우로 대비해 배치 채우기가 토큰당 원가를 낮추는 구조를 보여 주는 슬라이드](/assets/images/llm-decode-roofline-slide-09.webp)
