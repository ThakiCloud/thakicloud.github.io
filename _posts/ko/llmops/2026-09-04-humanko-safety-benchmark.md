...
이 질문도 왔습니다. "그래서 저희 모델이 EXAONE만큼 안전한가요?" 답은 "여덟 축 중 일곱은 구분되지 않고, 하나는 저희가 앞섭니다"입니다.

| | Human-KO | EXAONE | 차이 |
|---|---|---|---|
| KoBBQ 애매한 질문 정답률 | 85.7% | 77.5% | **+8.2%p** |
| KoBBQ 애매한 질문 고정관념 점수(답을 낸 것 중) | 81.6% | 68.9% | +12.7%p, 구분 불가 |
| BBQ(영어) 애매한 질문 고정관념 방향 | -26.3% | -15.2% | -11.2%p, 구분 불가 |
| 나머지 5개 세부 축 | | | 최소 검출 격차 미만 |

확실히 구분되는 축은 하나입니다. 한국어 애매한 맥락에서 Human-KO는 EXAONE보다 "모르겠다"를 8.2%p 더 자주 답합니다. 1,140문항씩 비교한 값이라 우연으로 보기 어렵습니다.

고정관념 점수 두 줄은 겉보기 차이가 커도 구분 불가로 적었습니다. 이 점수는 "모르겠다" 대신 답을 낸 문항만 놓고 계산하는 조건부 지표입니다. 한국어에서 답을 낸 문항은 Human-KO 163건, EXAONE 257건뿐이고, 영어에서는 38건과 33건입니다. 표본이 이렇게 작으면 의미 있다고 볼 수 있는 최소 차이가 한국어는 약 19%p, 영어는 60%p를 넘습니다. 관측된 12.7%p와 11.2%p는 그 아래입니다.

즉, 사람 말로는: 두 모델 중 어느 쪽이 "모르겠다"를 더 잘 고르는지는 확실히 알지만, 답을 낼 때 어느 쪽이 고정관념에 더 기우는지는 이 실험으로 알 수 없습니다.
...
이 비교의 한계도 그대로 적습니다. XSTest의 거절 판정은 심판 모델이 아니라 정해진 표현 사전으로 걸렀습니다. "죄송하지만 도와드릴 수 없습니다" 류의 뻔한 거절은 잘 잡지만, 에둘러 거절하거나 일부만 답하는 경우는 놓칠 수 있습니다. KoBBQ·BBQ 표본은 각각 2,280건과 1,500건으로 5~14%p보다 작은 차이는 이 실험으로 구분할 수 없습니다. 답을 낸 문항만 세는 고정관념 점수는 표본이 그보다 훨씬 작아서, 한국어는 약 19%p, 영어는 60%p가 넘는 차이만 구분합니다. EXAONE 비교는 지난 글과 같은 단서가 붙습니다. 저희와 아키텍처가 다르고 크기도 27B 대 33B로 다릅니다. 그리고 이 글의 모든 지표는 언어모델이 겉으로 뱉는 답을 기준으로 잽니다. 실제 위험한 사용 시나리오(반복 유도, 여러 턴에 걸친 우회)는 재지 않았습니다.

정정(2026-09-04): 처음 게시본은 고정관념 점수 두 줄을 구분되는 축으로 적었습니다. 조건부 지표에 전체 문항 수 기준의 최소 검출 격차를 붙인 계산 실수였고, 표와 문장을 바로잡았습니다.
