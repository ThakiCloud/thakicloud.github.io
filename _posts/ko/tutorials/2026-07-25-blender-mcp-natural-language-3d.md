...
```mermaid
flowchart TB
    User["사용자: 자연어로 장면 설명"] --> Model["모델 (Kimi K3 / Claude)"]
    Model --> Bridge["MCP 브리지<br/>양방향 통신"]
    Bridge --> Blender["Blender<br/>Python API 실행"]
    Blender --> Scene["3D 씬<br/>오브젝트·재질·조명"]
    Scene -. "현재 상태 회신" .-> Bridge
    Bridge -. "다음 동작 판단" .-> Model
    Scene --> Render["Eevee Next 렌더"]
```
