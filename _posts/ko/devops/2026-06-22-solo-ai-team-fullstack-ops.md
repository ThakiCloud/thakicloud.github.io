...
seo_description: "1인 AI 엔지니어가 스킬 1,620개, 서브에이전트 55개, 야간 자가진화 루프, haiku/sonnet/opus 비용 라우팅으로 거대한 AI 자동화 스택을 운영하는 방법. ThakiCloud Praxis 제품의 원천 경험 공개."
date: 2026-06-22
...
이 글은 그 운영 체계를 처음으로 전체적으로 공개합니다. 스킬 라우팅, 야간 진화, 비용 통제가 어떻게 하나의 운영체계로 맞물리는지, 그리고 이 경험이 어떻게 ThakiCloud Praxis 제품의 원천이 되는지를 설명합니다.
...
## 이것이 곧 제품: Praxis와 AI Platform

이 1인 운영 방식 자체가 ThakiCloud가 Praxis로 제품화하려는 것입니다. 자율 에이전트 런타임, 스킬 생태계, 자가진화, 거버넌스, 비용 통제를 엔지니어 누구나 쓸 수 있게 만드는 것이 목표입니다.
...
Praxis는 이 경험을 플랫폼으로 만드는 작업입니다. 운영자가 스킬을 정의하고, 에이전트를 구성하며, 비용 정책을 설정하면 나머지는 런타임이 처리하는 방식으로 진행합니다. AI Platform은 그 위에서 K8s 기반 워크로드 오케스트레이션(Kueue, ArgoCD)을 더합니다.
...
다음 글에서는 Praxis의 스킬 생태계 설계 원칙, 특히 thin harness와 fat skill의 구분이 왜 중요한지를 다룰 예정입니다.
