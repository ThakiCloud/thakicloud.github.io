---
title: "27B를 NVFP4로 줄여도 업무 에이전트 10종이 그대로 완주합니다"
excerpt: "27B를 NVFP4로 줄여 사내 GPU에 올리고 재무, 법무, 인프라 진단 등 업무 에이전트 10종을 화면 녹화로 돌렸습니다. 10건 모두 완주했고 지시 이행률은 상용 모델과 동률이며 같은 워크로드 비용은 Claude Opus 5의 123분의 1입니다. 대신 한국어 답변 다섯 건에 간체자가 섞였습니다. 이긴 축과 진 축을 모두 싣습니다."
seo_title: "NVFP4 양자화 27B로 업무 에이전트 10종 실행 실측 기록"
seo_description: "NVFP4 양자화 27B로 엔터프라이즈 에이전트 10종을 실행한 화면 녹화와 실측입니다. Claude Opus 5, Sonnet 5, GPT-5.6과의 비용 비교, 지시 이행률, 한국어 순도까지 이긴 축과 진 축을 함께 정리합니다."
date: 2026-08-22
...
자체 GPU에 모델을 올려 에이전트를 돌릴지 검토 중이라면 궁금한 것은 결국 하나입니다. 양자화로 줄인 모델이 데모가 아니라 실제 업무를 끝까지 해내느냐입니다. 27B 모델을 NVFP4로 줄여 사내 서버에 올리고 재무 마감부터 인프라 장애 진단까지 업무 에이전트 10종을 돌린 결과, 10건 모두 완주했고 단일 요청 지연은 양자화 전 bf16 원본보다 빨랐습니다.

같은 워크로드를 상용 API 정가로 환산하면 Claude Opus 5 기준 123배, GPT-5.6 Sol 기준 135배가 나옵니다. 지시 이행률은 Claude Sonnet 5, Haiku 4.5와 동률이었습니다. 대신 진 축도 있습니다. 한국어 답변 열 건 중 다섯 건에 간체자가 열한 자 섞였고 상용 모델 두 팔은 0건이었습니다. 아래에 양쪽을 다 싣습니다.
...
자체 모델로 돌렸다는 주장은 그 자체로는 검증이 안 됩니다. 화면만 봐서는 어느 백엔드가 답했는지 구분되지 않기 때문입니다.
...
데이터가 나가면 안 되는 조직에게 이 구분은 성능 문제가 아니라 규정 준수 문제입니다. 도구 안에서 일어나는 보조 호출까지 같은 백엔드에 머무르는지 확인할 수 있어야 자체 호스팅이라고 말할 수 있습니다.

## 세 제품이 만나는 지점

이 실행은 제품 하나로 이루어지지 않습니다.

에이전트를 정의하고 도구 권한을 부여하고 실행 루프를 돌리며 모든 도구 호출을 감사 기록으로 남기는 층이 **Paxis**입니다. 영상에서 보이는 에이전트 열 종, 샌드박스 실행, 도구 호출 추적이 전부 이 층의 산출물입니다.
...
그리고 그 27B를 NVFP4로 줄여 21기가바이트에 담아낸 것이 **Maxis** 계열의 양자화 작업입니다. 체크포인트가 우리 것이라는 사실이 중요한 이유는 성능 때문만이 아닙니다. 앞에서 본 간체자 혼입이 정확히 그 사례입니다. 서빙 설정을 아무리 만져도 그 결함은 안 없어지고, 학습 데이터와 정렬 단계에서 손봐야 합니다. 남의 API를 쓰면 그 결함을 발견해도 기다리는 것 말고 할 수 있는 일이 없습니다.

정리하면 업무를 자동화하는 것은 Paxis이고, 그것을 실행 가능하게 만드는 것이 Metis와 Maxis입니다. 세 층이 같은 조직 안에 있으면 모델을 바꿔 끼우고 그 효과를 측정하는 일이 실험이 아니라 운영이 됩니다.
...
자체 GPU에 모델을 올려 업무 에이전트를 돌리는 구성을 검토 중이시라면, 저 열 개 케이스 중 귀사 업무에 가장 가까운 것을 골라 같은 방식으로 재보시길 권합니다. 벤치마크 점수보다 그쪽이 훨씬 정직한 신호입니다.
