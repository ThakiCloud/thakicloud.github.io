...
과학적으로는 프리필-디코드 분리 문헌과 KV 캐시 계층화 문헌이 각각 성숙했음에도 둘을 하나의 변수 공간에서 같이 다룬 연구가 없었다는 공백을 메웁니다. 분리가 캐시의 가치를 높이는 동시에 가장 느린 계층의 가용성을 낮춘다는, 부호가 반대인 두 힘이 공존한다는 사실을 명시적으로 보여줍니다. 그리고 그 균형점이 하드웨어가 아니라 워크로드의 성질(재사용 꼬리 분포)로 결정된다는 것을 비용-지연시간 파레토 프론티어 논의에 추가된 축으로 제시합니다.
