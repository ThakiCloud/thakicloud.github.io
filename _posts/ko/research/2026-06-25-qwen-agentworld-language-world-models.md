...
또 하나 중요한 설계는 **네이티브 월드 모델**이라는 점입니다. 환경 모델링은 학습 첫 단계(CPT)부터 학습 목표였습니다. 후반에 덧붙인 부가 기능이 아니었습니다. 1천만 건이 넘는 실세계 상호작용 궤적을 활용해 처음부터 "환경을 모델링하는 능력"을 주입한 것입니다. 저자들의 표현을 빌리면, LLM은 환경에서 잘 행동하도록 학습되어 왔지만 환경 자체를 모델링하도록 학습된 적은 없었고 Qwen-AgentWorld가 그 빈 자리를 노립니다.
...
- **SFT(Supervised Fine-Tuning)**: 다음 상태 예측(next-state-prediction) 추론을 활성화합니다. 단순히 다음 상태를 맞히는 데 그치지 않습니다. 왜 그런 상태가 되는지를 사고 연쇄로 풀어내도록 만듭니다.
...
둘째, **온프레미스·자기호스팅 친화적입니다.** 35B-A3B는 Apache-2.0이라 상업적 활용과 자체 호스팅에 제약이 없습니다. 국내 공공·금융처럼 데이터가 외부로 나갈 수 없는 고객에게는, 고객 환경의 시뮬레이터를 우리 클러스터 안에 두는 선택지가 생깁니다. 외부 API에 맡기지 않아도 됩니다. 환경을 모델로 캡슐화하면 민감한 운영 환경을 외부에 노출하지 않고도 에이전트를 학습·평가할 수 있습니다.
...
둘째, **벤치마크의 성격**입니다. AgentWorldBench는 환경 예측의 충실도를 측정합니다. 에이전트의 과제 성공률을 재는 벤치마크가 아닙니다. 환경을 정확히 예측한다고 해서 그 환경에서 과제를 잘 푼다는 보장은 직접적이지 않습니다. 경로 2의 전이 결과가 그 연결을 일부 보여주지만 어디까지 일반화될지는 더 검증이 필요합니다.
