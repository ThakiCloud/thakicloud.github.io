...
![한쪽에는 정돈되지 않은 문서 더미가 흘러내리고 다른 쪽에는 작고 단정한 노드 묶음이 놓인 모습을 형상화한 추상 이미지](/assets/images/filesystem-memory-llm-agents-hero.png)
...
![폴더 구조의 모양이 자료 규모가 아니라 모델의 고유한 성향을 반영한다는 점을 설명한 슬라이드](/assets/images/filesystem-memory-llm-agents-slide-05.png)
...
![폴더 정리가 검색 비용은 절반으로 낮추지만 정답률에는 변화를 주지 못한다는 대비를 정리한 슬라이드](/assets/images/filesystem-memory-llm-agents-slide-03.png)
...
![관리 에이전트가 아니라 검색 에이전트 쪽에 좋은 모델을 붙이라는 결론을 정리한 슬라이드](/assets/images/filesystem-memory-llm-agents-slide-07.png)
...
![도구 하네스가 중립적 껍데기가 아니라 메모리 조직에 대한 제어 손잡이라는 점을 설명한 슬라이드](/assets/images/filesystem-memory-llm-agents-slide-09.png)
...
![메모리 계층별 마크다운 파일 수를 로그 스케일 막대로 비교한 그래프. 세션 원문 로그가 1789개로 압도적이고 나머지 정돈 계층은 모두 두 자릿수 이하](/assets/images/filesystem-memory-llm-agents-results.png)
...
![파일 1878개 중 95퍼센트가 정돈되지 않은 원문 로그이고 정돈된 계층은 45개에 그친다는 측정 결과를 정리한 슬라이드](/assets/images/filesystem-memory-llm-agents-slide-10.png)
