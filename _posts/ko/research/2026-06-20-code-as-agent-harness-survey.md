...
```mermaid
flowchart TB
    LLM["추론 엔진 LLM"] -->|"입출력·툴 시그니처"| L1
    subgraph HARNESS["코드 = 에이전트 하네스"]
        direction TB
        L1["1계층 하네스 인터페이스<br/>LLM·환경 양방향 정의"]
        L2["2계층 하네스 메커니즘<br/>계획 수립 + 적응 제어"]
        L3["3계층 멀티에이전트 조율<br/>분배·집약·의존성 관리"]
        L1 --> L2 --> L3
    end
    L1 -->|"실행 레이어"| ENV["환경<br/>파일시스템·API·DB·GUI·외부 서비스"]
```
