...
이 실험이 남긴 가장 실용적인 사실은 재료비입니다. 선호 쌍 2,757개와 정체성 문답 365개, 그리고 B200 한 장에 팔당 한 시간 반이면 결과가 나왔습니다. 데이터 생성, 오염 검사, 최소쌍 검증, 능력 회귀 게이트는 전부 코드가 판정합니다. 그래서 같은 절차를 다른 회사의 도메인에 그대로 다시 돌릴 수 있습니다. 순서는 다섯 단계입니다. 답하면 안 되는 질문과 반드시 답해야 하는 질문을 고객 도메인 평가셋으로 먼저 정의하고, 정체성 지도 학습으로 자기 출처를 가르치고, 안전 선호 학습을 얹고, 능력 게이트로 회귀를 재고, 카탈로그에 등록합니다. 즉, 사람 말로는 회사마다 자기 상담원을 2주 안에 새로 교육할 수 있다는 뜻입니다. 사람이 판단하는 곳은 첫 단계의 질문 정의 하나이고, 나머지는 같은 파이프라인이 돕니다.
