...
여기서 한 가지를 짚고 넘어가야 합니다. 생성 점수가 100을 넘었다고 단어장 학생이 더 낫다고 읽으면 안 됩니다. 합성 데이터는 권역끼리 실제보다 더 다르게 만들어졌습니다. 실제 사투리는 이웃 지역끼리 서로 섞입니다. 합성은 그 겹침을 재현하지 못했는데 점수가 그 차이를 이득처럼 읽었습니다.
...
그다음으로 효과가 큰 것은 원본 말뭉치를 키우는 일이 아니라 학습셋을 키우는 일이었습니다. 원본을 네 배로 늘렸을 때는 거의 그대로였습니다. 학습셋을 세 배로 늘리자 회수율이 뚜렷하게 올랐습니다.
