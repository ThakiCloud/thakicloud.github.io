...
excerpt: "브랜드 필름 한 편과 제품별 광고 일곱 편. 학습은 하지 않았고, 캐릭터 스틸 넉 장과 파이썬으로 만든 회색 판때기 한 편이 전부입니다."
categories:
  - research
tags:
  - video-generation
...
25초짜리 브랜드 필름입니다. 빈 유리 아트리움에서 시작해 일곱 마스코트가 각자의 세계에서 한 박자씩
맡습니다.
...
![브랜드 필름 여덟 샷]({{ site.url }}{{ site.baseurl }}/assets/images/cf-brand-film-shots.jpg)
*여덟 샷입니다. 세계는 매번 바뀌고 캐릭터는 각자 유지됩니다.*

이 편은 앞의 일곱 편과 만드는 방식이 다릅니다. 캐릭터를 조건으로 주는 것에 더해, 캐릭터가 어떻게
움직일지를 **동작 영상**으로 같이 넣었습니다. 그 영상은 레퍼런스 스틸 한 장을 파이썬으로 늘이고
줄여 만든 회색 판때기입니다. 배경도 소품도 없고 실루엣만 움직입니다. 세계는 전부 모델이 지었습니다.
...
## 동작 영상을 넣는 방식은 어떻게 나왔나

브랜드 필름에 쓴 방식은 세 번 실패한 뒤에 나왔습니다. 캐릭터가 움직이는 영상을 조건으로 넣는
경로가 원래 있었는데, 세 번 다 결과가 납작한 만화처럼 나왔고 원본 배경이 그대로 비쳤습니다. 이
스택으로는 안 되는 축이라고 적어 두었습니다.

틀린 판단이었습니다. 파이프라인에는 그 조건의 강도를 정하는 값이 있는데 세 번 모두 건드리지
않았습니다. 기본값이 최대치이고, 최대치에서는 모델이 지시 영역 바깥을 통째로 검게 지웁니다.
저희가 "배경이 샌다"고 읽은 그림이 사실은 세계를 지운 그림이었습니다. 강도를 0.6으로 낮추자
모델이 지시를 따르면서 세계도 지었습니다.

그다음 확인한 것이 이 방식이 캐릭터를 가리지 않는가입니다. 골격이 서로 다른 넷을 골라 두 동작씩
돌렸습니다. 둥근 물방울, 뾰족한 별, 바퀴 달린 사각, 납작한 연입니다. 여덟 편 모두 정체성을
유지했고 평균 0.715, 최저 0.670이 나왔습니다. 뼈대를 공유하지 않는 몸에 같은 동작이 옮겨 간
셈입니다.

한 번 더 틀린 것도 적어 둡니다. 그 여덟 편 중 네 편에서 프레임 일부가 검게 죽었고, 저희는 그것을
"다가오는 동작이 화면을 비운다"로 읽을 뻔했습니다. 그런데 도약은 밝은 온실에서, 다가오기는 어두운
밤거리에서 돌렸으니 동작과 장면을 함께 바꾼 셈이었습니다. 빠진 두 칸을 채워 보니 답이 갈렸습니다.
동작은 검은 프레임에 기여하지 않았고(12.4 대 12.3), 장면이 전부였습니다(낮 0.0 대 밤 24.7).
밝은 장면 여덟 칸은 예외 없이 0이었습니다.

브랜드 필름의 여덟 세계를 전부 대낮으로 잡은 것이 그래서입니다.

## 지표가 다섯 번 눈과 어긋났습니다

이 작업에서 가장 자주 걸린 함정은 모델이 아니라 지표였습니다.

정체성 지표는 프레임 전체 임베딩을 보기 때문에 색이 틀려도 잘 떨어지지 않습니다. 브랜드 필름에서
Paxis가 주황이 아니라 진갈색으로 나온 적이 있는데, 그 편의 정체성 점수는 0.731로 멀쩡했습니다.
결국 캐릭터 중앙부 RGB를 레퍼런스와 직접 재서 잡았습니다. 렌더가 (99, 52, 22), 레퍼런스가
(236, 137, 23), 거리 161이었습니다. 아트리움을 차가운 청백색으로 발주했더니 주황이 그 빛을
따라간 것이었고, 따뜻한 오후 햇빛으로 바꾸자 거리가 90으로 좁혀졌습니다.

수직 위치를 재서 점프를 따르는지 보려 한 적도 있습니다. 배경을 가장 흔한 밝기로 잡는 방식이라
온실 식물을 피사체로 집어서, 분명히 뛰고 있는 편을 "거의 안 움직인다"고 보고했습니다. 프레임을
눈으로 늘어놓고서야 알았습니다.
