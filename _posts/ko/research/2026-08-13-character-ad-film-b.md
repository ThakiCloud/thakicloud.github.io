...
>
> **2026-08-14 두 번째 정정: 배경 붕괴의 원인을 찾았습니다.** 위에서 "배경 붕괴는 관찰이라
> 유지한다"고 했는데, 그 관찰의 **원인**이 밝혀졌습니다. 학습 자체가 아니라 **학습 데이터**였습니다.
> 마스코트를 12개 서로 다른 장소에서 새로 찍어 나머지 조건을 전부 고정한 채 재학습했더니,
> 배경 붕괴 지표가 **0.906에서 0.791로 떨어져 zero-shot(0.792)과 같은 자리**에 왔습니다.
> 그리고 마스코트를 잃지도 않았습니다(피사체 유사도 변화 +0.004, p=0.67).
> 즉 아래 영상에서 여덟 장면이 하나로 뭉개진 것은 **그 어댑터의 성질이지 학습의 성질이 아닙니다.**
> 배경이 전부 같은 베이지 스튜디오였던 열두 클립으로 가르쳤으니, 모델 입장에서 "이 마스코트"와
> "이 배경"이 분리되지 않았던 겁니다. 배경을 다양하게 찍자 그 문제가 사라졌습니다.
> 그래서 이 글의 제목은 **틀렸습니다.** 학습이 진 게 아니라 **학습 데이터를 잘못 만든 것**이고,
> 그건 고칠 수 있는 문제였습니다.
