...
현재 LLM 기반 에이전트는 대부분 정적입니다. 프롬프트와 툴 정의는 사람이 직접 작성하고 실패가 반복되어도 에이전트 자체는 바뀌지 않습니다. 사람이 로그를 보고 프롬프트를 수정하고 다시 배포해야 비로소 다음 버전이 나옵니다. 이 사이클이 느리다는 사실은 누구나 압니다. 문제는 이를 어떻게 자동화하느냐입니다.
...
첫 번째 계층은 에이전트를 구성하는 모든 요소를 명시적 상태, 생명주기, 버전 인터페이스를 가진 "프로토콜 등록 리소스"로 모델링합니다. 프롬프트 하나도 버전이 붙은 아티팩트입니다. 특정 버전의 프롬프트가 어느 실행에 쓰였는지 추적 가능하고 롤백도 됩니다.

이 추상화가 실용적인 이유는 에이전트를 구성하는 요소들이 런타임에 교체 가능해지기 때문입니다. "프롬프트 v2가 v1보다 낫다"는 판단을 에이전트 자신이 내린다면, 재배포 없이 자체적으로 교체할 수 있습니다.

### Self Evolution Protocol Layer

두 번째 계층이 실제 자기진화를 담당합니다. 이 계층은 세 단계의 폐루프(closed-loop)를 돕니다.

1. **제안(Propose)**: 현재 성능 데이터를 바탕으로 개선 후보를 생성합니다.
2. **평가(Assess)**: 제안된 변경이 실제로 나아지는지 검증합니다.
3. **커밋(Commit)**: 검증을 통과한 변경만 프로덕션에 반영합니다.
...
ThakiCloud가 운영하는 K8s 기반 AI 플랫폼에서 AGP 아이디어가 가장 직접적으로 적용 가능한 곳은 에이전트 스킬 시스템입니다.

현재 `.claude/skills/` 아래에 있는 스킬들은 사람이 수동으로 작성하고 업데이트합니다. AGP의 Resource Substrate 모델을 적용하면, 각 스킬을 버전 관리된 아티팩트로 관리하고 실행 결과를 기반으로 스킬 개정 후보를 자동 생성하는 파이프라인을 구성할 수 있습니다. 이미 `selfharness-evolve` 워크플로가 야간에 스킬 진화를 시도하는 방향으로 가고 있고 AGP는 그 이론적 근거를 제공합니다.

멀티테넌트 플랫폼에서 한 가지 고려 사항이 있습니다. 자기진화 에이전트가 수정하는 대상이 공유 리소스라면, 한 테넌트의 실행에서 파생된 개선이 다른 테넌트에 영향을 줄 수 있습니다. 커밋 게이트를 테넌트 격리와 어떻게 연계할지는 설계 결정이 필요한 부분입니다.

## 마치며

AGP는 에이전트 시스템을 "코드처럼" 다루는 접근입니다. 코드에는 버전이 있고 테스트가 있고 배포 파이프라인이 있습니다. 에이전트의 프롬프트와 툴에도 같은 원칙을 적용하면, 사람이 개입하지 않아도 에이전트 품질이 점진적으로 올라갈 수 있습니다.
