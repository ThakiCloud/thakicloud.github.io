...
toc_icon: "cog"
toc_sticky: true
canonical_url: "https://thakicloud.com/tech-blog/ko/research/anthropic-economic-index-cadences/"
reading_time: true
categories:
  - research
...
이 보고서가 드러내는 흐름, 즉 AI가 업무를 얼마나 수행하는지를 측정하려는 방향 자체가 Paxis의 시장 포지셔닝 근거가 됩니다. Paxis는 ThakiCloud가 개발 중인 Agent-Native Cloud로, ai-platform(쿠버네티스 기반 AI/ML 인프라) 위에서 작동하는 에이전트 제어 평면입니다. Skills, Tools, Policies, Audit Logs를 일급 리소스로 다루며, 960개 이상의 스킬을 BM25로 선택하는 Skill Harness, 샌드박스 격리 실행, HKE 위키 지식엔진, DAG 멀티에이전트, NL Cron, MCP 커넥터, 자가진화 스킬, 그리고 정책게이트와 감사로그를 핵심 구성요소로 갖춥니다.

Anthropic이 제기하는 질문, 에이전트가 실제로 얼마나 많은 판단과 업무를 대신 수행하는지는 Paxis가 조직 안에서 답할 수 있는 질문입니다. Paxis의 정책게이트는 에이전트가 어떤 작업을 어떤 조건 아래 수행하는지를 제어하고, 감사로그는 모든 에이전트 행동을 기록합니다. 그 결과 조직 안에서 에이전트 노동이 어느 부서에서 얼마나 일어나는지를 정량화하고 거버넌스하는 데이터 기반이 생겨납니다. 측정 가능한 에이전트 워크포스를 제품이 직접 제공하는 것이고, 이는 사후에 채팅 로그에서 역으로 추정하는 방식과는 근본적으로 다릅니다.

ai-platform은 Paxis 위에서 작동하는 에이전트들이 사용하는 추론 인프라를 온프렘 또는 멀티테넌트 방식으로 공급합니다. 이 구조 덕분에 데이터를 외부로 내보내지 않고도 에이전트 활동 전체를 조직 경계 안에서 측정할 수 있습니다. Anthropic이 보여준 방향처럼 AI 영향을 로그가 아니라 실제 업무 산출물과 에이전트 행동 기록으로 설명하는 능력이, 엔터프라이즈 시장에서 Paxis가 만들 수 있는 차별점입니다.
