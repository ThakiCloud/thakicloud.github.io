...
![Headroom 파이프라인 다이어그램](/assets/images/headroom-reversible-context-compression-diagram.png)
*도구 출력 → Content-Type Router → SmartCrusher → 압축 컨텍스트 → LLM. 원본은 브레드크럼 해시와 TTL로 보관되어 가역 복원 경로를 유지합니다.*
