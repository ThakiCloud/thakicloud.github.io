...
toc_icon: "cog"
toc_sticky: true
canonical_url: "https://thakicloud.com/tech-blog/ko/dev/kueue-gpu-scheduling-preemption-patterns/"
reading_time: true
categories:
  - dev
...
Kubernetes 기본 스케줄러는 Pod이 이미 Running 상태가 되면 건드리지 않습니다. PriorityClass로 Pending Pod 간 순서를 정할 수는 있지만, 실행 중인 낮은 우선순위 Job을 밀어내는 로직이 없습니다.

Kueue는 Workload라는 추상 레이어를 Pod 위에 올립니다. 스케줄러가 아니라 워크로드 큐 관리자 역할을 합니다. ClusterQueue가 쿼터를 정의하고, Kueue가 어떤 Workload를 언제 Admit할지, 누군가의 Workload를 Preempt할지를 결정합니다.
...
ClusterQueue는 팀이나 프로젝트 단위로 쿼터를 설정하는 오브젝트입니다. GPU, CPU, 메모리를 리소스 플레이버별로 할당합니다.
...
서빙 엔드포인트는 중단이 곧 장애입니다. `PreemptLowerPriority` 정책을 달고, 트래픽 급증 시 하위 우선순위 학습 Pod을 즉시 회수할 수 있어야 합니다.
...
연구자가 Jupyter 세션이나 단기 실험을 돌리는 워크로드입니다. 훈련보다 반응속도가 중요하지만 서빙보다는 낮습니다.
...
긴 학습 Job은 가장 먼저 선점 대상이 됩니다. 체크포인트 저장 간격을 짧게 유지하면 선점으로 인한 손실을 최소화할 수 있습니다.
...
Borrowing은 쿼터를 낭비하지 않으면서 버스트 용량을 확보하는 메커니즘입니다. inference-team이 8개 GPU를 할당받았지만 4개만 쓰고 있다면, training-team이 그 4개를 빌릴 수 있습니다.
...
inference-team이 새 작업을 제출하면 Kueue가 training-team에서 빌려간 GPU를 회수합니다. 이때 회수 정책이 `reclaimWithinCohort: LowerPriority`면 낮은 우선순위 작업부터 선점합니다.
...
이 조합이 없으면 로깅 수집기, 프록시, 모니터링 에이전트 같은 CPU 바운드 데몬셋이 GPU 노드의 자원을 점유합니다.

## MultiKueue: 멀티클러스터 작업 분산

2026년 Kueue 로드맵에서 주요 기능으로 올라와 있는 MultiKueue는 현재 베타 상태로 기본 활성화되어 있습니다. 관리 클러스터(manager)가 작업을 받아 워커 클러스터들로 분산합니다.
...
현재 단계에서는 `terminationGracePeriodSeconds`를 충분히 길게 잡고, 학습 코드에서 SIGTERM 핸들러로 체크포인트를 저장하는 방식으로 유사한 효과를 냅니다.
...
협력적 선점이 정식 지원되면 체크포인트 완료를 Kueue가 확인한 뒤 새 워크로드를 Admit하는 흐름이 될 것입니다.

## 실무에서 자주 마주치는 함정

**함정 1: 선점 정책을 프로덕션 투입 전에 검증하지 않습니다.** 개발 클러스터에서 선점 시나리오를 한 번 실제로 돌려봐야 합니다. PDB, 유예 시간, 체크포인트 저장 시간의 조합이 예상대로 동작하는지 확인해야 합니다.

**함정 2: Cohort 없이 borrowingLimit을 설정합니다.** Cohort에 묶이지 않은 ClusterQueue는 borrowingLimit을 설정해도 빌릴 대상이 없습니다.
