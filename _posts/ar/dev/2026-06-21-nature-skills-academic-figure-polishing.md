...
```mermaid
flowchart LR
    FC["Figure Contract (تحديد الخلاصة)"] --> BE{"بوابة الخلفية: Python أم R؟"}
    BE -->|Python| PY["matplotlib rcParams"]
    BE -->|R| RR["ggplot2"]
    PY --> STYLE["تطبيق rcParams + PALETTE"]
    RR --> STYLE
    STYLE --> EXP["SVG / TIFF قابل للتحرير"]
    EXP --> QA["عقد QA"]
```
...
![شكل من لوحتين بأسلوب Nature يقارن زمن الاستجابة والإنتاجية بين FP16 وINT8](/assets/images/nature-skills-results.webp)
