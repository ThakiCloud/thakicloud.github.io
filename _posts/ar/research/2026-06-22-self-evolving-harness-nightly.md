...
```mermaid
flowchart TD
    A["🌙 00:00 launchd trigger"] --> B["المرحلة 1: Weakness Mining<br/>استخراج آثار الإخفاقات في 24 ساعة الماضية<br/>تجميع حسب نوع الإخفاق"]
    B --> C["المرحلة 2: Harness Proposal<br/>إنتاج تصحيحات دنيا لكل فئة إخفاق<br/>فروق أحادية الاهتمام، مقترحات متنوعة"]
    C --> D["المرحلة 3: Proposal Validation<br/>التحقق من اجتياز بوابة الانحدار<br/>التطبيق على SKILL.md فقط عند النجاح وغياب الانحدار"]
    D --> E{هل اجتازت البوابة؟}
    E -- "نجاح" --> F["✅ تحديث تلقائي لـ SKILL.md<br/>(مع نقطة تفتيش shadow-git)"]
    E -- "فشل / لا دليل" --> G["🛑 ABORT<br/>لا تُطبَّق أي تحويلات"]
    F --> H["🌅 اليوم التالي يبدأ<br/>بحزام محسَّن"]
    G --> H
```
