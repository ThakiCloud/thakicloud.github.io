...
```mermaid
flowchart TB
    A["مستند الإدخال<br/>(PDF، صورة)"] --> B["SAM-ViT<br/>استخراج الميزات"]
    B --> C["CLIP-ViT<br/>ضغط الرموز 16×"]
    C --> D["رموز المرجع البصري<br/>(256 رمزًا لكل صفحة)"]
    D --> E{"انتباه R-SWA"}
    F["نافذة منزلقة<br/>النص المُولَّد مؤخرًا"] --> E
    E --> G["فك تشفير MoE<br/>3B معامل / ~500M نشط"]
    G --> H["ذاكرة KV ثابتة<br/>(مستقلة عن الطول)"]
    G --> I["النص المخرج<br/>(Markdown / منظَّم)"]
    H -.محفوظة.-> E
```
