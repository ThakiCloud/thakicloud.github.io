...
```mermaid
flowchart TB
    A[طلب المهندس<br/>نموذج غير محدد] --> B[بوابة LLM]
    B --> C{سياسة الإعداد الافتراضي}
    C -->|الإعداد الحالي| D[نموذج حدودي مكلف<br/>تكلفة رمز مرتفعة]
    C -->|بعد التغيير| E[إعداد افتراضي مفتوح الأوزان<br/>GLM 5.2 / Kimi 2.7]
    E --> F[توجيه حسب صعوبة المهمة]
    F -->|مهام بسيطة متكررة| G[نموذج اقتصادي]
    F -->|مهام عالية الصعوبة| H[نموذج حدودي<br/>اختيار صريح]
    B --> I[البحث في الذاكرة المؤقتة]
    I -->|إصابة| J[استجابة مخزنة<br/>رموز 0]
    I -->|إخفاق| F
    G --> K[تسطيح الإنفاق]
    H --> K
    J --> K
