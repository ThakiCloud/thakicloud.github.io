...
toc_sticky: true
lang: ar
canonical_url: "https://thakicloud.com/tech-blog/ar/llmops/nvidia-glm-5-2-nvfp4/"
reading_time: true
categories:
  - llmops
---

![صورة تجريدية لشبكة عصبية بدقة 16 بت تتكثّف في نواة مدمجة بدقة 4 بت](/assets/images/nvidia-glm-5-2-nvfp4-hero.webp)
...
![مخطط لاستراتيجية التكميم الانتقائي ومسار الخدمة في GLM-5.2-NVFP4](/assets/images/nvidia-glm-5-2-nvfp4-diagram.webp)
...
![رسم بياني شريطي يقارن خط أساس FP8 ودرجات معايير NVFP4](/assets/images/nvidia-glm-5-2-nvfp4-results.webp)
...
منصة ai-platform من ThakiCloud تجدول أحمال عمل GPU عبر Kueue على Kubernetes وتخدم النماذج بـ vLLM. نموذج مُعتمَد مسبقاً مثل `nvidia/GLM-5.2-NVFP4` يؤثر على هذه المنظومة بشكل مباشر. حين تنزل 753 مليار معامل إلى 4 بت وتتسع لعُقدة Blackwell واحدة بـ 8 وحدات GPU، يصبح إنشاء تجمّع ResourceFlavor مخصص في Kueue لاستدلال NVFP4 خياراً عملياً، ويتحوّل هامش ذاكرة GPU المكتسب مباشرة إلى زيادة في عدد المستأجرين المخدومين معاً. للعملاء في القطاع العام والمالية حيث لا يمكن إخراج البيانات خارج الحدود المؤسسية، يشكّل تقديم الاستدلال الرائد داخلياً ميزة تنافسية للمنصة بحد ذاتها. وبما أن NVIDIA اعتمدت هذا النموذج عبر ModelOpt، يمكن للفريق التركيز على التوجيه متعدد المستأجرين والتوسّع التلقائي ومراقبة التكلفة بدلاً من تشغيل خط أنابيب التكميم من الصفر. كما يمكن اعتماد الوصفة المطبّقة على GLM-5.2 كمعيار داخلي لتكميم نماذج MoE: خبراء التوجيه بـ 4 بت، والخبير المشترك والانتباه بـ BF16.

Paxis هو مستوى التحكم في الوكلاء (agent control plane) الذي يعمل فوق ai-platform. يعامل المهارات والأدوات والسياسات وسجلات التدقيق كموارد من الدرجة الأولى، ويضمّ Skill Harness يغطي أكثر من 960 مهارة مع تنفيذ معزول في بيئة رملية وتنسيق وكلاء متعدد عبر DAG وموصلات MCP وبوابة سياسات مع سجل تدقيق. يستخدم Paxis ملف models.yaml مصدراً وحيداً للحقيقة (SSOT) للتنقل بين النماذج، لذا حين تصبح نقطة وصول GLM-5.2-NVFP4 متاحة على ai-platform، يمكن لوكلاء Paxis استهلاكها دون أي تعديل في الكود. كل نقطة وصول رائدة تُنشأ داخلياً وتحلّ محل استدعاء API خارجي مدفوع تحسّن اقتصاديات الوحدة لكل مهارة. وكلما قدّمت ai-platform طاقة استدلال أكبر بتكلفة أقل، اتّسع نطاق عمل وكلاء Paxis معها.
