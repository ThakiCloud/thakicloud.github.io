...
```mermaid
flowchart TB
    subgraph ROLLOUT["الـ Rollout (SGLang + Router)"]
        SGL["محرك استدلال SGLang<br/>توليد الاستجابات"]
        RT["sgl-router<br/>نقطة وصول واحدة متوافقة مع OpenAI"]
        RW["المكافأة / verifier<br/>حساب الدرجات"]
    end
    subgraph BUFFER["Data Buffer (الجسر)"]
        DB["تهيئة الموجّهات<br/>بيانات مخصصة<br/>إدارة استراتيجية توليد الـ Rollout"]
    end
    subgraph TRAIN["التدريب (Megatron-LM)"]
        MG["حلقة تدريب Megatron<br/>تحديث الأوزان"]
    end

    RT --> SGL
    SGL --> RW
    RW -->|"بيانات التوليد + سجل المكافأة"| DB
    DB -->|"توفير دفعات التدريب"| MG
    MG -.->|"مزامنة المعاملات"| SGL
```
