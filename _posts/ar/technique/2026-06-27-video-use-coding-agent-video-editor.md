...
```mermaid
flowchart TB
    A[مجلد الفيديو الأصلي<br/>+ تعليمة جملة واحدة] --> B[العامل: تفكيك النية]
    B --> C[تحرير القص<br/>تحديد الأقسام]
    B --> D[حذف الكلمات الحشو<br/>تحليل الصوت]
    B --> E[توليد الترجمات<br/>التعرف على الصوت]
    B --> F[تدرج الألوان<br/>توحيد النبرة]
    B --> G[تراكب الرسوم المتحركة]
    G --> G1[عامل فرعي 1<br/>HyperFrames]
    G --> G2[عامل فرعي 2<br/>Remotion]
    G --> G3[عامل فرعي 3<br/>Manim / PIL]
    C --> H[تجميع الخط الزمني]
    D --> H
    E --> H
    F --> H
    G1 --> H
