...
toc_sticky: true
lang: ar
canonical_url: "https://thakicloud.github.io/ar/agentops/nvidia-verified-agent-skills/"
reading_time: true
categories:
  - agentops
published: false
...
```mermaid
flowchart TB
    A[المستودع المصدر] --> B[المراجعة]
    B --> C[الفحص الأمني<br/>SkillSpector]
    C --> D[التقييم]
    D --> E[بطاقة المهارة]
    E --> F[توقيع OMS]
    F --> G[الإدراج في الكتالوج]
    G --> H[المزامنة اليومية]
    H -. تنزيل .-> V{التحقق من التوقيع<br/>model_signing}
    V -->|تطابق التجزئة| OK[تم التحقق<br/>يستمر النشر]
    V -->|عدم تطابق| NG[رُصد تلاعب<br/>حُظر النشر]
```
