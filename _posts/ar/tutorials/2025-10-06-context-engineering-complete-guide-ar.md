...
-->
<div class="d3-arch" data-arch-root id="gineeringcompleteguidear-1"></div>
<style>
...
<script>
  (() => {
    ...
    const bootstrap = () => {
      const container = document.getElementById('gineeringcompleteguidear-1')
        || document.querySelector('.d3-arch[data-arch-root]:not([data-mounted])');
      if (!container || (container.dataset && container.dataset.mounted === 'true')) return;
      if (container.dataset) container.dataset.mounted = 'true';

      try {
        const uid = 'gineeringcompleteguidear-1';
