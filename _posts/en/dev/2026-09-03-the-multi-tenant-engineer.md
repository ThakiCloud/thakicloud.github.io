...
Second, the database layer. PostgreSQL offers [RLS, row-level security](https://www.postgresql.org/docs/current/ddl-rls.html). You set a policy on each table: a row is visible only when the table's tenant column matches a session variable, and the application sets that variable when it connects. RLS does not replace application scoping. It is a backstop. A bug that bypasses the ORM still lands on a connection where RLS applies. A connection that failed to set the session variable sees nothing, and that is the safe direction to fail.
