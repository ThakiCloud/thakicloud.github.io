---
title: "I Just Founded the Sixth Engineering Discipline"
excerpt: "Five disciplines were founded this year. Our loop ran on the same machine the whole time."
date: 2026-07-31
categories:
  - comics
tags:
...
[Download video](/assets/videos/posts/만화/the-sixth-engineering-discipline/comic.mp4)

## What this means for ThakiCloud

Whatever the label becomes, the last question stays the same: whose machine is that loop running on? ThakiCloud decided to answer that one first. Paxis handles the orchestration, spawning agents and feeding results back until things converge, while Metis keeps the GPUs and the data those loops run on inside your own facility. On-prem simply means it runs in your rack instead of someone else's datacenter, so training data and logs never leave the building. This blog runs on exactly that setup. Picking the news, drafting the script, rendering the art and the video is one in-house agent loop, and the name for that loop changed five times this year while the rack stayed where it was. If chasing the vocabulary is wearing you out, start by checking who owns the hardware underneath it.
