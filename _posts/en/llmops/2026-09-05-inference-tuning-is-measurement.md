...
seo_description: "A 27B model quantized to 4-bit on one B200, measured from 1 to 256 concurrent requests. Matching quantization, speculative decoding and memory settings to the workload holds 100 tokens/sec per request at 32 concurrent users."
date: 2026-09-05
published: true
categories:
  - llmops
...
**Raising the concurrency limit cost no memory either.** We took the setting from 8 to 32, a factor of four, and the cache moved by 0.03 percent. This model reserves that space up front.
...
**Spare memory is margin, not waste.** The engine offers to spend every remaining byte, and we left 17 GB alone. Without that cushion, an unusually large request does not slow the server down. It kills it.

In plain terms: pushing a setting to its maximum and tuning it correctly are different jobs.

## The numbers

Measured on one card after tuning, in output tokens per second, median of five runs per step.
