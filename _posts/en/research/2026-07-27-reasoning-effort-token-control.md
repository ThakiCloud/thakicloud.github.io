...
toc_label: "Contents"
toc_icon: "cog"
audiobook: "https://drive.google.com/file/d/1M2Qr74b5GfSgy4N1j9M1--DkhSG8FTDZ/view"
...
![Illustration of the core idea of Does a Reasoning-Effort Label Actually Cut Compute? What Qwen3-8B Showed Us](/assets/images/reasoning-effort-token-control-hero.webp)
...
<div class="mermaid">
flowchart TB
    gen["Model generates response<br/>Reasoning effort: low"] --> check{"Correct answer"}
    check -- "no" --> zero["Reward 0<br/>length term never fires"]
    check -- "yes" --> paid["Reward 1.0<br/>plus 0.5 if length near target"]
    zero --> nosignal["Length signal vanishes<br/>keeps reasoning long"]
    paid --> signal["Length signal active<br/>incentive to answer short"]
    nosignal --> MATH["MATH-500<br/>short usually means wrong<br/>separation 1.08x"]
    signal --> GSM["GSM8K<br/>can be short and correct<br/>separation 1.28x"]
</div>
*The length reward is contingent on correctness, so on hard problems where short means wrong, the length term disappears entirely.*
...
- Basis for the hard-budget fix: [L1: Controlling How Long A Reasoning Model Thinks With Reinforcement Learning](https://arxiv.org/abs/2503.04697)
- The RL algorithm used: [DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models](https://arxiv.org/abs/2402.03300)
- Model trained: [Qwen3-8B](https://huggingface.co/Qwen/Qwen3-8B)
- Evaluation set (hard math): [HuggingFaceH4/MATH-500](https://huggingface.co/datasets/HuggingFaceH4/MATH-500)
- Evaluation set (arithmetic): [Training Verifiers to Solve Math Word Problems](https://arxiv.org/abs/2110.14168)
- Reasoning-effort parameter spec: [OpenAI Reasoning models documentation](https://developers.openai.com/api/docs/guides/reasoning)
