...
The most practical thing this experiment left behind is the bill of materials. The result came from 2,757 preference pairs, 365 identity pairs, and about an hour and a half on a single B200 per arm. Data generation, contamination checks, minimal-pair validation, and the capability regression gate are all judged by code. That is why the same procedure can be rerun on another company's domain as is. It runs in five steps: define the questions the model must refuse and the questions it must answer as a customer-domain evaluation set, teach the model its own origin with identity supervised finetuning, add safety preference training, measure regression with the capability gate, and register the result in the catalog. In plain terms, any company can retrain its own agent in two weeks. The only step that needs human judgment is the first one, defining the questions; the rest is the same pipeline running again.
